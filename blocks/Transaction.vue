        ...
        <div v-else-if="openedTab==='main'" class="tileBlock">
            ...
            <i-row class="_margin-top-1">
                ...
                <i-column v-else xs="12" :md="canSaveContact?7:12">
                    <i-button block link variant="secondary" @click="openedTab='contactsList'">{{isOwnAddress?'Own account':choosedContact.name}}&nbsp;&nbsp;<i class="far fa-angle-down"></i></i-button>
                </i-column>
                <i-column xs="12" md="5">
            ...
            <i-input v-model="inputTotalSum" size="lg" placeholder="0.00" type="number">
                <i-button v-if="!choosedToken" slot="append" block link variant="secondary" @click="openedTab='tokenList'">Select token</i-button>
                <i-button v-else slot="append" class="selectedTokenBtn" block link variant="secondary" @click="openedTab='tokenList'">{{choosedToken.symbol}}&nbsp;&nbsp;<i class="far fa-angle-down"></i></i-button>
            </i-input>
            ...
            <i-button block size="lg" variant="secondary" class="_margin-top-1" :disabled="!inputTotalSum || !choosedToken || inputTotalSum>transactionMaxAmount" @click="commitTransaction()">{{type==='withdraw'?'Withdraw':'Transfer'}}</i-button>
...
export default {
  ...
  methods: {
    ...
    commitTransaction: async function () {
      ...
      try {
        this.mainLoading = true;
        if (!this.inputTotalSum || !this.choosedToken || this.inputTotalSum > this.transactionMaxAmount || !this.isAddressValid) {
          return (this.mainLoading = false);
        }
    ...
    transfer: async function (syncWallet, syncProvider) {
      ...
      const transferTransaction = await syncWallet.syncTransfer({
        ...
        fee: zksync.closestPackableTransactionFee(
          ethers.BigNumber.from(syncWallet.provider.tokenSet.parseToken(this.choosedToken.symbol, this.feesObj.normal.toString()).toString()),
        ),
      });
      this.transactionAmount = parseFloat(this.inputTotalSum);
