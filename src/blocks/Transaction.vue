<template>
  <div class="transactionPage">
    ...
    <i-modal v-model="transferWithdrawWarningModal" class="prevent-close" size="md">
      ...
      <div>
        <div class="_padding-bottom-1">
          You are about to transfer money to an address that doesn't have a zkSync balance yet. The transfer will happen inside zkSync L2. If you want to move money from zkSync to the mainnet, please use the
    ...
    <loading-block v-if="loading===true" :headline="transactionTypeName">
      <a
          v-if="transactionInfo.hash" :href="transactionInfo.explorerLink" class="_display-block _text-center"
          target="_blank"
      >
    ...
    >
      <p v-if="transactionInfo.type==='ActivateAccount'" class="_text-center _margin-top-0">
        Your account has been successfully unlocked and now you can proceed to {{ type }}.
      </p>
      <p v-else class="_text-center _margin-top-0">
    ...
    <div v-else class="transactionTile tileBlock">
      ...
      <amount-input ref="amountInput" v-model="inputtedAmount" :maxAmount="maxAmount" :token="chosenToken?chosenToken:undefined" :type="type" autofocus @chooseToken="chooseTokenModal=true" @enter="commitTransaction()"/>

      <i-radio-group v-if="chosenToken && type==='withdraw' && (!chosenFeeToken || chosenFeeToken.symbol===chosenToken.symbol) && feesObj" v-model="transactionMode" class="_margin-top-2">
      ...
      <p v-if="!ownAccountUnlocked" class="tileTextBg _margin-top-1">To start using your account you need to register your public key once. This operation costs
...
export default Vue.extend({
  ...
  computed: {
    ...
    enoughFeeToken: function (): boolean {
      ...
      if (this.cantFindFeeToken || !this.inputtedAddress || !this.chosenToken || !this.feesObj || !this.chosenFeeToken || this.feesLoading || this.chosenFeeToken.symbol === this.chosenToken.symbol) {
        return true
      }
      // @ts-ignore: Unreachable code error
  ...
  methods: {
    ...
    transfer: async function (): Promise<void> {
      ...
      const txAmount = utils.parseToken((this.chosenToken as Balance).symbol, this.inputtedAmount)
      // @ts-ignore: Unreachable code error
      const transferTransaction = await transaction(this.inputtedAddress, (this.chosenToken as Balance).symbol, this.feeToken.symbol, txAmount.toString(), this.feesObj.normal, this.$store) as Transaction
      this.transactionInfo.amount.amount = txAmount.toString()
      this.transactionInfo.amount.token = this.chosenToken as Balance
      this.transactionInfo.fee.token = this.feeToken
