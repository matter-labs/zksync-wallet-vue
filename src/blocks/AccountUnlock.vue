<template>
  <div v-if="!loading && !transactionInfo.success" class="tileBlock">
    <i-modal v-model="chooseTokenModal" size="md">
      ...
      <choose-token only-allowed @chosen="chooseToken($event)" />
    </i-modal>
    ...
    <i-input size="lg" placeholder="Select token" disabled :value="inputVal">
      <i-button v-if="!chosenToken" slot="append" block link variant="secondary" @click="chooseTokenModal = true"> Select token </i-button>
      <i-button v-else slot="append" class="selectedTokenBtn" block link variant="secondary" @click="chooseTokenModal = true"
    ...
    <i-button :disabled="!chosenToken || !enoughFeeToken || !totalFee || feesLoading" block class="_margin-top-1" size="lg" variant="secondary" @click="unlock()"
      ><i class="far fa-lock-open-alt"></i> Unlock
    </i-button>
  ...
  <loading-block v-else-if="loading" headline="Unlock account">
    <a v-if="transactionInfo.hash" class="_display-block _text-center" target="_blank" :href="transactionInfo.explorerLink">
      Link to the transaction <i class="fas fa-external-link" />
    </a>
    <p v-if="tip" class="_display-block _text-center">{{ tip }}</p>
  </loading-block>
  <success-block v-else-if="transactionInfo.success === true" headline="Unlock account" :fee="transactionInfo.fee" :tx-link="transactionInfo.explorerLink">
...
export default Vue.extend({
  ...
  methods: {
    ...
    async getUnlockPrice(): Promise<void> {
      if (!this.chosenToken) {
        return;
      }
    ...
    setTransactionInfo(transaction: Transaction): void {
