...
export default Vue.extend({
  ...
  methods: {
    ...
    getTransactionExplorerLink(transaction: ZkInTx): string {
      return (
        (transaction.tx.type === "Deposit" ? `${this.$accessor.config.network.ethBlockExplorerUrl}/tx` : `${this.$accessor.config.network.zkSyncBlockExplorerUrl}/transactions`) +
    ...
    async getWithdrawalTx() {
      ...
      if (singleTx && (singleTx.tx.type === "Withdraw" || singleTx.tx.type === "WithdrawNFT")) {
        ...
        if (txFromStore) {
          this.ethTx = `${this.$accessor.config.network.ethBlockExplorerUrl}/tx/${txFromStore}`;
        } else {
          const syncProvider = walletData.get().syncProvider;
          const ethTx = await syncProvider!.getEthTxForWithdrawal(singleTx.hash);
          if (ethTx) {
            this.ethTx = `${this.$accessor.config.network.ethBlockExplorerUrl}/tx/${this.$options.filters!.formatTxHash(ethTx)}`;
